...
export default function PlayerCardPage() {
  ...
  return <div className="page"><div className="page-head"><div><p className="eyebrow">Earn ratings</p><h1>Player Card</h1></div><div className="actions"><Link className="button" to="/workout-planner">Train</Link><Link className="button secondary" to="/tests">Test</Link></div></div>{card && <div className="grid-2"><PlayerCard card={card} /><div className="stack"><Panel title="Rating Breakdown"><div className="stack">{labels.map(([key, label]) => <div key={key}><p className="between muted"><span>{label}</span><b>{card[key]}</b></p><progress value={card[key]} max={99} /></div>)}</div></Panel><Panel title="Progression"><div className="stat-grid"><div className="stat"><strong>{card.progression.workouts}</strong><small>Training Sessions</small></div><div className="stat"><strong>{card.progression.tests}</strong><small>Test Blocks</small></div><div className="stat"><strong>{card.overall}</strong><small>Overall Rating</small></div></div><p style={{ marginTop: '1rem' }}>{card.progression.nextFocus}</p></Panel><Panel title="Recovery Load"><BodyRecoveryMap map={card.progression.recoveryMap} /></Panel></div></div>}</div>;
}
