...
export default function ChallengesPage() {
  ...
  return <div className="page"><div className="page-head"><div><p className="eyebrow">Verified XP Objectives</p><h1>Objectives</h1></div><span className="score-chip">{done} / {items.length || 0} Complete</span></div><Panel title="No Manual Claims"><p className="muted">Objectives cannot be typed in by hand. XP is only awarded when MatchFit records a completed programme workout, sport session, fitness test, or Google Health wearable sync.</p></Panel><div className="grid-2"><Panel title="Daily Reset"><strong>{timeLeft(resets.daily)}</strong><p className="muted">Daily objectives reset at midnight UK time.</p></Panel><Panel title="Weekly Reset"><strong>{timeLeft(resets.weekly)}</strong><p className="muted">Weekly objectives run Monday to Sunday and reset at midnight UK time.</p></Panel></div><h2>Daily Objectives</h2><div className="grid-2">{daily.map((item) => <Card item={item} key={item.id} />)}</div><h2>Weekly Objectives</h2><div className="grid-2">{weekly.map((item) => <Card item={item} key={item.id} />)}</div></div>;
}
